...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Function GET_ALL_SCHEMAS
--------------------------------------------------------
...
/
--------------------------------------------------------
--  DDL for Function GET_APPLY_LAG
--------------------------------------------------------
...
from DBA_APPLY_PROGRESS@offdb p, dba_apply@offdb a where p.apply_name = a.apply_name and a.apply_name !=''OGG$C_COOLOF'' ';
  ...
  END IF;  
    ...
    sql_text := 'SELECT ATLAS_DBMON.APPLYLAG_OBJ(SOURCE,APPLY_LAG,SNAPSHOT_TIME,STATUS) 
           FROM (' || sql_text || ')';
                   DBMS_OUTPUT.put_line(sql_text);
...
/
--------------------------------------------------------
--  DDL for Function GET_AWR_STATS4_SQLID
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_AWR_STATS4_SQLID" (db_name VARCHAR2, sql_id VARCHAR2)
RETURN awr_tab AS 
  -- The AUTONOMOUS_TRANSACTION and the rollback at the end of the proc are neccessary to close 
  -- the transaction open by Oracle because of the used DB link.
  PRAGMA AUTONOMOUS_TRANSACTION;
...
/
--------------------------------------------------------
--  DDL for Function GET_BLOCKING_SESSIONS
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_BLOCKING_SESSIONS" (db VARCHAR2, fromDate VARCHAR2 DEFAULT NULL, toDate VARCHAR2 DEFAULT NULL)
...
BEGIN
  ...
  IF (fromDate IS NULL AND toDate IS NULL) THEN
    where_clause :=  'T_STAMP = (SELECT MAX(T_STAMP) FROM ATLAS_DBMON.DB_BLOCKING_SESSIONS)';
  ELSE
  ...
  END IF;
    ...
    FOR i IN 1..roots.count LOOP
        ...
        /*SELECT atlas_dbmon.block_sess_obj(BLOCKING_SESS_SID, BLOCKED_SESS_ID, DEPTH,
...
/
--------------------------------------------------------
--  DDL for Function GET_DB_UP_STATUS
--------------------------------------------------------
...
/
--------------------------------------------------------
--  DDL for Function GET_EXP_PLAN_DATA
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_EXP_PLAN_DATA" (sql_id VARCHAR2, db VARCHAR2) 
...
BEGIN  
  ...
  sql_text := 'BEGIN 
                DBMS_APPLICATION_INFO.SET_MODULE@'||db||'(module_name => ''dbmon'', action_name => ''get exec_plan''); 
               END;';
...
/
--------------------------------------------------------
--  DDL for Function GET_JOBS_INFO
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_JOBS_INFO" (db VARCHAR2, dbschema VARCHAR2)RETURN job_info_tab AS 
  -- The AUTONOMOUS_TRANSACTION and the rollback at the end of the proc are neccessary to close 
  -- the transaction open by Oracle because of the used DB link.
  PRAGMA AUTONOMOUS_TRANSACTION;
...
DBMS_OUTPUT.put_line(sql_text);
  sql_text := 'SELECT ATLAS_DBMON.JOB_INFO_OBJ(OWNER, JOB_NAME, JOB_CLASS, LAST_START_DATE, LAST_RUN_DURATION, LAST_STATUS, CURRENT_STATE, NEXT_RUN_TIME, REPEAT_INTERVAL, INFO) 
           FROM (' || sql_text || ')';
...
/
--------------------------------------------------------
--  DDL for Function GET_SCHEMA_DETAILS
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_SCHEMA_DETAILS" (schema_name IN VARCHAR2, db_name IN VARCHAR2) 
...
BEGIN
  ...
  stmt := 'SELECT USERNAME, nvl(RESP_NAME,''no info'') RESP_NAME, nvl(RESP_EMAIL, ''no info'') RESP_EMAIL, ACCOUNT_STATUS, 
            ...
            ORDER BY 1';
  DBMS_OUTPUT.put_line(stmt);
  stmt := 'SELECT ATLAS_DBMON.SCHEMA_DETAILS_OBJ(USERNAME, RESP_NAME, RESP_EMAIL, ACCOUNT_STATUS, PASSWORD_EXPIRY_DATE, EGROUP) 
           FROM (' || stmt || ')';
...
/
--------------------------------------------------------
--  DDL for Function GET_SCHEMA_SESS_DETAILS
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_SCHEMA_SESS_DETAILS" (db_name VARCHAR2, user_name VARCHAR2) 
...
BEGIN
  ...
  sql_text := 'SELECT ATLAS_DBMON.schema_sess_details_obj(INST_ID, SID, STATUS, OSUSER, PROCESS, MACHINE, PROGRAM, 
...
/
--------------------------------------------------------
--  DDL for Function GET_SESS_DISTRIBUTION_DATA
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_SESS_DISTRIBUTION_DATA" (db VARCHAR2)
...
BEGIN
  ...
  sql_text := 'SELECT ATLAS_DBMON.sess_distr_obj(node_id, username, num_active, num_sess, sess_limit, t_stamp)
                FROM
...
/
--------------------------------------------------------
--  DDL for Function GET_SESSIONS_CHART_DATA
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_SESSIONS_CHART_DATA" (db VARCHAR2, schema_name VARCHAR2, fromDate VARCHAR2, toDate VARCHAR2, node NUMBER)
RETURN sess_distr_tab AS 
  -- The AUTONOMOUS_TRANSACTION and the rollback at the end of the proc are neccessary to close 
  -- the transaction open by Oracle because of the used DB link.
  PRAGMA AUTONOMOUS_TRANSACTION;
...
/
--------------------------------------------------------
--  DDL for Function GET_STORAGE_DATA
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_STORAGE_DATA" (db VARCHAR2, schema_name VARCHAR2, selected_year NUMBER)
...
/
--------------------------------------------------------
--  DDL for Function GET_TOP10_PER_SCHEMA
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_TOP10_PER_SCHEMA" (db VARCHAR2, node NUMBER, schema_name VARCHAR2, fromDate VARCHAR2, toDate VARCHAR2)
RETURN TOP10_TAB AS
  -- The AUTONOMOUS_TRANSACTION and the rollback at the end of the proc are neccessary to close 
  -- the transaction open by Oracle because of the used DB link.
  PRAGMA AUTONOMOUS_TRANSACTION;
...
BEGIN
  ...
  --SELECT DBNODES into node_num FROM ATLAS_DBMON.DATABASES WHERE UPPER(DBNAME) = UPPER(db);
  ...
  --FOR i IN 1..node_num
    --loop
    idxj := 0;    
    for j in ( SELECT METRIC_NAME FROM ATLAS_DBMON.DB_METRIC_DESC WHERE METRIC_ID >= 20000 )
      loop
      idxj := idxj + 1;
  ...
  stmt := 'SELECT ATLAS_DBMON.TOP10_OBJ(INST_ID,CHART_TYPE,PARSING_SCHEMA_NAME,SQLID,METRIC_VALUE,METRIC_UNIT,SQL_TEXT) 
           FROM (' || stmt || ')';
...
/
--------------------------------------------------------
--  DDL for Function GET_TOP10_PER_SCHEMA_ALLNODES
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_TOP10_PER_SCHEMA_ALLNODES" (db VARCHAR2, schema_name VARCHAR2, fromDate VARCHAR2, toDate VARCHAR2)
RETURN TOP10_TAB AS
  -- The AUTONOMOUS_TRANSACTION and the rollback at the end of the proc are neccessary to close 
  -- the transaction open by Oracle because of the used DB link.
  PRAGMA AUTONOMOUS_TRANSACTION;
...
/
--------------------------------------------------------
--  DDL for Function GET_TOP10_SESSIONS_DATA
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_TOP10_SESSIONS_DATA" (db VARCHAR2, fromDate VARCHAR2, toDate VARCHAR2)
  RETURN TOP10_TAB AS
  -- The AUTONOMOUS_TRANSACTION and the rollback at the end of the proc are neccessary to close 
  -- the transaction open by Oracle because of the used DB link.
  PRAGMA AUTONOMOUS_TRANSACTION;
...
/
--------------------------------------------------------
--  DDL for Function TEST_CONNECTION
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."TEST_CONNECTION" (schema_name IN VARCHAR2, db_name IN VARCHAR2) 
...
/
--------------------------------------------------------
--  DDL for Function TOP10_TEST
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."TOP10_TEST" (db VARCHAR2, node NUMBER, schema_name VARCHAR2, fromDate VARCHAR2, toDate VARCHAR2)
RETURN TOP10_TAB
AUTHID CURRENT_USER
AS
tab TOP10_TAB:= TOP10_TAB();
