...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "DBMON_SCHEMA"."GET_AWR_STATS4_SQLID" (db_name VARCHAR2, sql_id VARCHAR2)
RETURN awr_tab AS 
  -- The AUTONOMOUS_TRANSACTION and the rollback at the end of the proc are neccessary to close 
  -- the transaction open by Oracle because of the used DB link.
  PRAGMA AUTONOMOUS_TRANSACTION;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "DBMON_SCHEMA"."GET_BLOCKING_SESSIONS" (db VARCHAR2, fromDate VARCHAR2 DEFAULT NULL, toDate VARCHAR2 DEFAULT NULL)
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "DBMON_SCHEMA"."GET_EXP_PLAN_DATA" (sql_id VARCHAR2, db VARCHAR2) 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "DBMON_SCHEMA"."GET_JOBS_INFO" (db VARCHAR2, dbschema VARCHAR2)RETURN job_info_tab AS 
  -- The AUTONOMOUS_TRANSACTION and the rollback at the end of the proc are neccessary to close 
  -- the transaction open by Oracle because of the used DB link.
  PRAGMA AUTONOMOUS_TRANSACTION;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "DBMON_SCHEMA"."GET_SCHEMA_DETAILS" (schema_name IN VARCHAR2, db_name IN VARCHAR2) 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "DBMON_SCHEMA"."GET_SCHEMA_SESS_DETAILS" (db_name VARCHAR2, user_name VARCHAR2) 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "DBMON_SCHEMA"."GET_SESS_DISTRIBUTION_DATA" (db VARCHAR2)
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "DBMON_SCHEMA"."GET_SESSIONS_CHART_DATA" (db VARCHAR2, schema_name VARCHAR2, fromDate VARCHAR2, toDate VARCHAR2, node NUMBER)
RETURN sess_distr_tab AS 
  -- The AUTONOMOUS_TRANSACTION and the rollback at the end of the proc are neccessary to close 
  -- the transaction open by Oracle because of the used DB link.
  PRAGMA AUTONOMOUS_TRANSACTION;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "DBMON_SCHEMA"."GET_STORAGE_DATA" (db VARCHAR2, schema_name VARCHAR2, selected_year NUMBER)
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "DBMON_SCHEMA"."GET_TOP10_PER_SCHEMA" (db VARCHAR2, node NUMBER, schema_name VARCHAR2, fromDate VARCHAR2, toDate VARCHAR2)
RETURN TOP10_TAB AS
  -- The AUTONOMOUS_TRANSACTION and the rollback at the end of the proc are neccessary to close 
  -- the transaction open by Oracle because of the used DB link.
  PRAGMA AUTONOMOUS_TRANSACTION;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "DBMON_SCHEMA"."GET_TOP10_PER_SCHEMA_ALLNODES" (db VARCHAR2, schema_name VARCHAR2, fromDate VARCHAR2, toDate VARCHAR2)
RETURN TOP10_TAB AS
  -- The AUTONOMOUS_TRANSACTION and the rollback at the end of the proc are neccessary to close 
  -- the transaction open by Oracle because of the used DB link.
  PRAGMA AUTONOMOUS_TRANSACTION;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "DBMON_SCHEMA"."GET_TOP10_SESSIONS_DATA" (db VARCHAR2, fromDate VARCHAR2, toDate VARCHAR2)
  RETURN TOP10_TAB AS
  -- The AUTONOMOUS_TRANSACTION and the rollback at the end of the proc are neccessary to close 
  -- the transaction open by Oracle because of the used DB link.
  PRAGMA AUTONOMOUS_TRANSACTION;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "DBMON_SCHEMA"."TEST_CONNECTION" (schema_name IN VARCHAR2, db_name IN VARCHAR2) 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE FUNCTION "DBMON_SCHEMA"."TOP10_TEST" (db VARCHAR2, node NUMBER, schema_name VARCHAR2, fromDate VARCHAR2, toDate VARCHAR2)
RETURN TOP10_TAB
AUTHID CURRENT_USER
AS
tab TOP10_TAB:= TOP10_TAB();
