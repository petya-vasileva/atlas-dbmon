...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PACKAGE "ATLAS_DBMON"."DBMON_PKG" AS
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PACKAGE BODY "ATLAS_DBMON"."DBMON_PKG" AS
    ...
    BEGIN
        ...
        IF(upper(db_name)= 'ALL')THEN
            SELECT
                atlas_dbmon.schema_obj(username)
            BULK COLLECT
            INTO sql_tab
            FROM
                (
    ...
    BEGIN
        IF(upper(db_name)= 'ATLR')THEN
            ...
            from DBA_APPLY_PROGRESS@atlr p, dba_apply@atlr a where p.apply_name = a.apply_name and a.apply_name !=''OGG$C_COOLOF'' '
            ;
    ...
    BEGIN
        sql_text := 'SELECT basic_metrics_OBJ(metric, unit, threshold, visible, cnt_nodes, Node1, Node2, Node3, Node4)
                    ...
                    FROM
                        atlas_dbmon.db_hist_metrics        dbhm
                        JOIN atlas_dbmon.db_metric_desc         dbmd ON dbhm.metric_id = dbmd.metric_id
                        JOIN atlas_dbmon.db_metrics_threshold dbmt ON(dbhm.metric_id = dbmt.metric_id
    ...
    BEGIN
        ...
        sql_text := 'SELECT atlas_dbmon.exp_plan_obj(plan_table_output)
              FROM (SELECT plan_table_output 
                    FROM ATLAS_DBMON_R.QUERY_EXPLAIN_PLAN@'
                    || db_name
                    || ')';
        EXECUTE IMMEDIATE sql_text BULK COLLECT
        INTO sql_tab;
  ...
  -- the transaction open by Oracle because of the used DB link.
    ...
    BEGIN
        ...
        stmt := 'SELECT USERNAME, nvl(RESP_NAME,''no info'') RESP_NAME, nvl(RESP_EMAIL, ''no info'') RESP_EMAIL, ACCOUNT_STATUS, 
            ...
            ORDER BY 1';
        dbms_output.put_line(stmt);
        stmt := 'SELECT ATLAS_DBMON.ACCOUNT_INFO_OBJ(USERNAME, RESP_NAME, RESP_EMAIL, ACCOUNT_STATUS, PASSWORD_EXPIRY_DATE, EGROUP) 
           FROM ('
                || stmt
                || ')';
                ...
                FROM
                  ...
                  FROM
                    ...
                    || '''
                      AND t_stamp   =
                        (SELECT MAX(T_STAMP) 
                        FROM ATLAS_DBMON.DB_SESS_DISTR WHERE DB_NAME = '''
                    || upper(db_name)
                    || '''
                        )
  ...
  -- the transaction open by Oracle because of the used DB link.
        ...
        stmt := 'Select af.USERNAME
                ...
                from (
                    ...
                    (
                        ...
                        from table(ATLAS_DBMON.GET_SESS_DISTRIBUTION_DATA('''
                || db_name
                || '''))
                    )
                    PIVOT
    ...
    -- dbms_output.put_line('============');
    --dbms_output.put_line(sql_text);
        stmt := 'SELECT ATLAS_DBMON.COMPACT_SESS_DISTR_OBJ(username, node1, node2, node3, node4, num_active_sess, 
  ...
  --SELECT DBNODES into node_num FROM ATLAS_DBMON.DATABASES WHERE UPPER(DBNAME) = UPPER(db_name);
        SELECT
            COUNT(*)
        INTO met_cnt
        FROM
            atlas_dbmon.db_metric_desc
        WHERE
            metric_id >= 20000;
  ...
  --end loop;
    ...
    FUNCTION get_schema_sess_distribution(
      ...
      -- the transaction open by Oracle because of the used DB link.
        ...
        sql_text   VARCHAR2(4000);
    BEGIN
        sql_text := 'SELECT ATLAS_DBMON.schema_sess_distr_obj(node_id, username, num_active, num_sess, osuser, machine, program)
                    FROM
                      (
                    ...
                    || '''), ''%'' ) 
                        ...
                        FROM ATLAS_DBMON.DB_SESS_DISTR WHERE DB_NAME = UPPER('''
                    || db_name
                    || '''))
                        ORDER BY 4 desc
                      )';
  ...
  --end loop;
        ...
        stmt := 'SELECT ATLAS_DBMON.TOP10_OBJ(INST_ID,CHART_TYPE,PARSING_SCHEMA_NAME,SQLID,ROUND(METRIC_VALUE,1),METRIC_UNIT,SQL_TEXT) 
           FROM ('
                || stmt
                || ')';
        dbms_output.put_line(stmt);
  ...
  -- the transaction open by Oracle because of the used DB link.
    ...
    and to_date(''31-12-'
        ...
        stmt := 'SELECT ATLAS_DBMON.TOP10_TABLES_OBJ(DT, TABLE_SIZE_GB, OBJECT_TYPE, OWNER, OBJECT_NAME ) 
                 FROM ('
                    || stmt
                    || ')
