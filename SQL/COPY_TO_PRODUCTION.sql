...
SELECT 
   REPLACE (DBMS_METADATA.GET_DDL( object_type, object_name, 'DBMON_SCHEMA'), 'CREATE OR REPLACE TYPE "DBMON_SCHEMA"', 'CREATE OR REPLACE TYPE "DBMON_SCHEMA_DEV"') as definition
--	 REPLACE(DBMS_METADATA.GET_DDL( object_type, object_name, 'DBMON_SCHEMA'),'" AS' , '@OFFDB" AS' ) as definition
FROM 
   user_objects 
WHERE 
   object_type IN ('TYPE')
