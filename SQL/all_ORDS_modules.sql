...
-- Schema: DBMON_SCHEMA   Date: Tue Jul 02 10:39:47 EEST 2019
--
BEGIN
  ORDS.ENABLE_SCHEMA(
      p_enabled             => TRUE,
...
'SELECT * FROM TABLE (DBMON_SCHEMA.DBMON_PKG.get_blocking_sessions(:db, :fromDate, :toDate))'
  ...
  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'db',
      p_pattern        => 'compact_streams/:db/:metric/:fromDate/:toDate',
      p_priority       => 0,
      p_etag_type      => 'HASH',
      p_etag_query     => NULL,
      p_comments       => NULL);
  ORDS.DEFINE_HANDLER(
      ...
      p_comments       => NULL,
      p_source         => 
'SELECT inst_id, t_stamp, metric_value FROM DBMON_SCHEMA.DB_HIST_METRICS 
WHERE T_STAMP >= TO_DATE((:fromDate), ''YYYY-MM-DD"T"HH24:MI'') 
AND T_STAMP < TO_DATE((:toDate), ''YYYY-MM-DD"T"HH24:MI'')
AND dbname = UPPER(:db) AND METRIC_ID = :metric 
order by 1, 2'
      );
  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'db',
...
'select * from table(DBMON_SCHEMA.dbmon_pkg.GET_DB_UP_STATUS(:db))'
  ...
  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'db',
      p_pattern        => 'streams/:db/:metric/:fromDate/:toDate',
      p_priority       => 0,
      p_etag_type      => 'HASH',
      p_etag_query     => NULL,
      p_comments       => NULL);
