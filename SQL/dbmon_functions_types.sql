  ...
  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."NODETABLE" AS OBJECT
( 
  nodenumber    number(10)
);

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."APPLYLAG_OBJ" AS OBJECT
(
...
status  		VARCHAR2(32 BYTE)  
  ...
  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."TOP10_TABLES_OBJ" AS OBJECT
( DT VARCHAR2(20),
  ...
  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."TOP10_OBJ" AS OBJECT
(
INST_ID	NUMBER(1,0), 
...
SQL_TEXT	VARCHAR2(4000 BYTE) 
  ...
  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."STORAGE_OBJ" AS OBJECT
( DT VARCHAR2(10),
  INDEX_SIZE NUMBER,
  TABLE_SIZE NUMBER);

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."SESS_DISTR_OBJ" AS OBJECT
( node_id NUMBER,
  ...
  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."SCHEMA_SESS_DETAILS_OBJ" AS OBJECT
( INST_ID NUMBER,
  SID NUMBER,
  STATUS VARCHAR2(8)  ,
  OSUSER VARCHAR2(30),
  ...
  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."SCHEMA_OBJ" AS OBJECT
( schema_name VARCHAR2(30));

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."SCHEMA_DETAILS_OBJ" AS OBJECT 
(
  USERNAME VARCHAR2(30),
  ...
  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."JOB_INFO_OBJ" AS OBJECT
( OWNER	VARCHAR2(30), 
...
INFO	VARCHAR2(4000) 
  ...
  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."EXP_PLAN_OBJ" AS OBJECT
( plan_table_output VARCHAR2(4000));

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."DB_UP_OBJ" AS OBJECT
(
status  		VARCHAR2(1000 BYTE)  
);

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."BLOCK_SESS_OBJ" AS OBJECT
( BLOCKING_SESS_ID NUMBER,
  ...
  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."AWR_OBJ" AS OBJECT
( inst NUMBER,
  ...
  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."APPLYLAG_TAB" AS TABLE OF ATLAS_DBMO
N.APPLYLAG_OBJ;

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."AWR_TAB" AS TABLE OF awr_obj;

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."BLOCK_SESS_TAB" AS TABLE OF block_se
ss_obj;

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."DB_UP_TAB" AS TABLE OF ATLAS_DBMON.D
B_UP_OBJ;

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."EXP_PLAN_TAB" AS TABLE OF exp_plan_o
bj;

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."JOB_INFO_TAB" AS TABLE OF job_info_o
bj;

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."SCHEMA_DETAILS_TAB" AS TABLE OF SCHE
MA_DETAILS_OBJ;

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."SCHEMA_SESS_DETAILS_TAB" AS TABLE OF
 schema_sess_details_obj;

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."SCHEMA_TAB" AS TABLE OF schema_obj;

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."SESS_DISTR_TAB" AS TABLE OF sess_dis
tr_obj;

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."STORAGE_TAB" AS TABLE OF storage_obj
;

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."TOP10_TAB" AS TABLE OF ATLAS_DBMON.T
OP10_OBJ;

  CREATE OR REPLACE TYPE "ATLAS_DBMON_DEV"."TOP10_TABLES_TAB" AS TABLE OF TOP10_
TABLES_OBJ;

  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_TOP10_SESSIONS_DATA" (db VARCHAR
...
BEGIN
    ...
    SELECT DBNODES into node_num FROM ATLAS_DBMON.DATABASES WHERE UPPER(DBNAME) 
= UPPER(db);
    
    SELECT count(*) into met_cnt
    FROM ATLAS_DBMON.DB_METRIC_DESC  
    WHERE METRIC_ID >= 20000;
  
    for i in 1..node_num
      loop
      idxj := 0;    
      for j in ( SELECT METRIC_NAME FROM ATLAS_DBMON.DB_METRIC_DESC WHERE METRIC
_ID >= 20000 )
        loop
        idxj := idxj + 1;
...
s sqlid,  AVG(metric_value) metric_value,  metric_unit, db_name
                    FROM ATLAS_DBMON.CHART_METRICS
...
,  metric_unit, db_name
                  ...
                  WHERE ROWNUM <= 10) s
                  JOIN  ATLAS_DBMON.SQL_TEXT_HIST h 
    ...
    stmt := 'SELECT ATLAS_DBMON.TOP10_OBJ(INST_ID,CHART_TYPE,PARSING_SCHEMA_NAME
...
END GET_TOP10_SESSIONS_DATA;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_SESS_DISTRIBUTION_DATA" (db VARC
HAR2)
...
BEGIN
  ...
  sql_text := 'SELECT ATLAS_DBMON.sess_distr_obj(node_id, username, num_active, 
...
, SUM(num_sess) num_sess, t_stamp
                      ...
                      AND t_stamp   =
                        (SELECT MAX(T_STAMP) 
                        FROM ATLAS_DBMON.DB_SESS_DISTR WHERE DB_NAME = '''||uppe
r(db)||'''
                        )
                      ...
                      FROM gv$instance@'||db||' gv,
                        atlas_dbmon.DB_SESS_DISTR dbmon
                      WHERE DB_NAME = '''||upper(db)||'''
                      AND t_stamp   =
                        (SELECT MAX(T_STAMP) 
                        FROM ATLAS_DBMON.DB_SESS_DISTR WHERE DB_NAME = '''||uppe
...
END GET_SESS_DISTRIBUTION_DATA;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_EXP_PLAN_DATA" (sql_id VARCHAR2,
 db VARCHAR2) 
...
; 
  ...
  sql_text := 'SELECT atlas_dbmon.exp_plan_obj(plan_table_output)
              FROM (SELECT plan_table_output 
                    FROM ATLAS_DBMON_R.QUERY_EXPLAIN_PLAN@'||db||')';
...
END GET_EXP_PLAN_DATA;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_STORAGE_DATA" (db VARCHAR2, sche
...
SIZE_MB TABLE_SIZE
                  FROM ATLAS_DBMON.DB_VOLUME_SCHEMES
...
where_clause||'
  ...
  WRAPPER := 'SELECT ATLAS_DBMON.STORAGE_OBJ(TO_CHAR(trunc(DT, ''MM''), ''MON-YY
''), 
...
END GET_STORAGE_DATA;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_ALL_SCHEMAS" (db VARCHAR2) 
...
BEGIN
  ...
  IF (UPPER(db) = 'ALL') THEN
    SELECT ATLAS_DBMON.schema_obj(username) bulk collect into sql_tab
                   FROM (SELECT DISTINCT username 
                           FROM ATLAS_DBMON.SCHEMA_DETAILS 
                       ORDER BY 1);    
  ELSE 
    sql_text := 'SELECT ATLAS_DBMON.schema_obj(username)
                   FROM (
                 SELECT username 
                   FROM dba_users@'||db||'
...
END GET_ALL_SCHEMAS;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_SCHEMA_DETAILS" (schema_name IN 
...
'no info'') EGROUP
            ...
            LEFT JOIN ATLAS_DBMON.SCHEMA_RESPONSIBLES ON (USERNAME = SCHEMA_NAME
) 
            WHERE (username LIKE UPPER('''||s_name||'''))
            ORDER BY 1';
  DBMS_OUTPUT.put_line(stmt);
  stmt := 'SELECT ATLAS_DBMON.SCHEMA_DETAILS_OBJ(USERNAME, RESP_NAME, RESP_EMAIL
...
END GET_SCHEMA_DETAILS;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_SCHEMA_SESS_DETAILS" (db_name VA
...
BEGIN
  ...
  sql_text := 'SELECT ATLAS_DBMON.schema_sess_details_obj(INST_ID, SID, STATUS, 
...
END GET_SCHEMA_SESS_DETAILS;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."TOP10_TEST" (db VARCHAR2, node NUMBE
...
= UPPER(db);
  ...
  SELECT count(*) into met_cnt
  FROM ATLAS_DBMON.DB_METRIC_DESC WHERE METRIC_ID >= 20000;

  --FOR i IN 1..node_num
    --loop
    idxj := 0;    
    for j in ( SELECT METRIC_NAME FROM ATLAS_DBMON.DB_METRIC_DESC WHERE METRIC_I
D >= 20000 )
      loop
      idxj := idxj + 1;
...
ode || ' AND parsing_schema_name = ''' || schema_name || ''' 
 ...
 metric_unit, db_name
  ...
  stmt := 'SELECT ATLAS_DBMON.TOP10_OBJ(INST_ID,CHART_TYPE,PARSING_SCHEMA_NAME,S
...
END TOP10_TEST;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_TOP10_PER_SCHEMA" (db VARCHAR2, 
...
END GET_TOP10_PER_SCHEMA;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_BLOCKING_SESSIONS" (db VARCHAR2,
...
BEGIN
  ...
  IF (fromDate IS NULL AND toDate IS NULL) THEN
    where_clause :=  'T_STAMP = (SELECT MAX(T_STAMP) FROM ATLAS_DBMON.DB_BLOCKIN
G_SESSIONS)';
  ELSE
...
logontime
                FROM
                    (
                        ...
                        FROM
                            atlas_dbmon.db_blocking_sessions
                        WHERE
...
ion
        /*SELECT atlas_dbmon.block_sess_obj(BLOCKING_SESS_SID, BLOCKED_SESS_ID, 
DEPTH,
...
N_TIME,
            ...
            FROM
              (SELECT BLOCKING_SESS_SID,
                ...
                BLOCKING_ON_ROW_ADDRESS,
                blocking_sess_logontime
              FROM ATLAS_DBMON.DB_BLOCKING_SESSIONS stmt1
              WHERE '||where_clause||'
              AND db_name                       = UPPER('''||db||''')
...
END GET_BLOCKING_SESSIONS;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_AWR_STATS4_SQLID" (db_name VARCH
AR2, sql_id VARCHAR2)
RETURN awr_tab AS 
  -- The AUTONOMOUS_TRANSACTION and the rollback at the end of the proc are necc
essary to close 
...
BEGIN  
  ...
  sql_text := 'SELECT ATLAS_DBMON.awr_obj(inst,
                                            begin_time,
                                            plan_hash_value,
                                            module,
                                            parsing_schema_name,
...
END GET_AWR_STATS4_SQLID;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."TEST_CONNECTION" (schema_name IN VAR
...
'no info'') EGROUP
            ...
            ORDER BY 1';
  --DBMS_OUTPUT.put_line(stmt);
  --stmt := 'SELECT ATLAS_DBMON.SCHEMA_DETAILS_OBJ(USERNAME, RESP_NAME, RESP_EMA
...
END TEST_CONNECTION;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_SESSIONS_CHART_DATA" (db VARCHAR
...
END GET_SESSIONS_CHART_DATA;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_JOBS_INFO" (db VARCHAR2, dbschem
...
DBMS_OUTPUT.put_line(sql_text);
  sql_text := 'SELECT ATLAS_DBMON.JOB_INFO_OBJ(OWNER, JOB_NAME, JOB_CLASS, LAST_
...
END GET_JOBS_INFO;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_TOP10_PER_SCHEMA_ALLNODES" (db V
...
_value,  metric_unit, db_name from
                ...
                SELECT inst_id,  chart_type,  parsing_schema_name,  sql_id as sq
lid,  AVG(metric_value) metric_value,  metric_unit, db_name
                FROM ATLAS_DBMON.TOP_SQL_PER_DB_SCHEMA
                WHERE DB_NAME = UPPER('''|| db ||''')
                AND T_STAMP >= TO_DATE('''|| fromDate ||''', ''YYYY-MM-DD"T"HH24
:MI:SS'') AND T_STAMP < TO_DATE('''|| toDate ||''', ''YYYY-MM-DD"T"HH24:MI:SS'')
...
END GET_TOP10_PER_SCHEMA_ALLNODES;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_APPLY_LAG" (db VARCHAR2) 
...
_STATS@'||db||' WHERE NAME = ''apply lag'' AND value IS NOT NULL';
  ...
  END IF;  
    ...
    sql_text := 'SELECT ATLAS_DBMON.APPLYLAG_OBJ(SOURCE,APPLY_LAG,SNAPSHOT_TIME,
STATUS) 
           FROM (' || sql_text || ')';
                   DBMS_OUTPUT.put_line(sql_text);
...
END GET_APPLY_LAG;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_DB_UP_STATUS" (db VARCHAR2) 
...
BEGIN
    ...
    sql_text := 'SELECT ATLAS_DBMON.DB_UP_OBJ(STATUS) 
...
END GET_DB_UP_STATUS;
  ...
  CREATE OR REPLACE FUNCTION "ATLAS_DBMON"."GET_TOP10_TABLES_PER_SCHEMA" (db VAR
...
FOR j in (
	SELECT OBJECT_NAME, OBJECT_TYPE, SIZE_MB FROM ( 
		...
		FROM ATLAS_DBMON.DB_VOLUME_OBJECTS   
...
stmt := 'SELECT ATLAS_DBMON.TOP10_TABLES_OBJ(DT, TABLE_SIZE_GB, OBJECT_TYPE, OWN
ER, OBJECT_NAME ) 
           FROM (' || stmt || ')';
