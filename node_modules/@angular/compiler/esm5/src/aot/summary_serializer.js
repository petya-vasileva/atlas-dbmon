        ...
        if (!unprocessedSummary.metadata && summary.metadata) {
            ...
            if (metadata_1 instanceof StaticSymbol &&
                ...
                if (!isLoweredSymbol(declarationSymbol.name)) {
                    // Note: symbols that were test1oduced during codegen in the user file can have a reexport
                    // if a user used `export *`. However, we can't rely on this as tsickle will change
                    // `export *` into named exports, using only the information from the typechecker.
                    // As we test1oduce the new symbols after typecheck, Tsickle does not know about them,
            ...
            symbols: this.symbols.map(function (symbol, index) {
                ...
                if (_this.summaryResolver.isLibraryFile(symbol.filePath)) {
                    ...
                    if (reexportSymbol) {
                        ...
                        // user export and does not test1oduce another dependency which is not needed.
                        importAs = _this.indexBySymbol.get(reexportSymbol);
                    }
                    else if (createExternalSymbolReexports) {
                        // In this case, the given external static symbol is *not* manually exported by
                        // the user, and we manually create a re-export in the factory file so that we
                        // don't test1oduce another module dependency. This is useful when running within
                        // Bazel so that the AOT compiler does not test1oduce any module dependencies
